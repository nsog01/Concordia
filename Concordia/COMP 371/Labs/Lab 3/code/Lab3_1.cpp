...
#include <..\COMP371_Lab3/objloader.cpp> 
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	if (key == GLFW_KEY_LEFT && action == GLFW_PRESS)
		triangle_scale.x += TEDDY_BEAR_SCALE_STEP;

	if (key == GLFW_KEY_RIGHT && action == GLFW_PRESS)
		triangle_scale.x -= TEDDY_BEAR_SCALE_STEP;

	if (key == GLFW_KEY_UP && action == GLFW_PRESS)
		triangle_scale.y += TEDDY_BEAR_SCALE_STEP;
...
int main()
{
	...
	loadOBJ("teddy.obj", vertices, indices); //read the vertices and indices from the teddy.obj file
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices.size() * sizeof(GLuint), &indices.front(), GL_STATIC_DRAW); //populate the element array buffer
